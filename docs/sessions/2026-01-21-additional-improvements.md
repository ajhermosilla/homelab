...
4. **Validation**: Added NFS mount verification and secrets permissions enforcement in `docker-compose-deploy.yml`. Pi-hole webpassword validation already exists in `pihole.yml`.
5. **Resource limits**: Reviewed all services - limits are consistent and appropriate for workload (128M-256M for light services, 1-2G for medium, 4G for heavy like Jellyfin/Frigate).
6. **Frigate config**: `frigate.yml` already exists at `docker/fixed/docker-vm/security/frigate.yml` - marked complete.
7. **Backup verification**: Enhanced `restic-backup.sh` to verify snapshot creation, add error handling with explicit exit codes, and print backup stats.
8. **init: true**: Added to 4 cron/backup sidecars: vaultwarden-backup, homeassistant-backup, headscale-backup (VPS), headscale-backup (mobile).
9. **Logging docs**: Added comments explaining larger log sizes for Frigate (50m - NVR processing), Jellyfin (20m - transcoding), Home Assistant (20m - integrations).
10. **Secrets permissions**: Added tasks to `docker-compose-deploy.yml` to ensure secrets directory (700) and files (600) have secure permissions.
11. **NFS verification**: Added tasks to `docker-compose-deploy.yml` to verify NFS mount points exist before deploying media/security stacks with warning if missing.
...
12. **env_file paths**: Verified all paths are correct - different depths reflect actual directory structure (2-4 levels based on location).
13. **Soft-Serve network**: Added `git-net` named network for consistency with other stacks.
14. **ntfy base URL**: Made configurable via `${NTFY_BASE_URL:-https://notify.cronova.dev}` environment variable.
...
15. **First-time setup guide**: Already exists at `docs/setup-runbook.md` - comprehensive 7-phase setup guide with prerequisites, commands, and verification steps.
16. **Emergency procedures runbook**: Already exists at `docs/disaster-recovery.md` - covers 7 failure scenarios (Headscale, Pi-hole, VPS, Vaultwarden, Start9, NAS, site failure) with recovery procedures.
17. **Deployment order**: Created `docs/deployment-order.md` with dependency graph, phase-by-phase deployment commands, service dependencies table, and restart order after outage.
